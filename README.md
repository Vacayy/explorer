...
- **종목 발견**: 종목 발견 화면에서 50개 전략 도구를 선택하거나 자연어로 조건을 조합해 종목을 찾습니다. 신고가·이평·MACD·SONAR·Stochastic·순위 조건과 기존 역헤드앤숄더 분석을 지원하며, 읽기 전용 스냅샷에서 계산한 근거·미평가 범위·차트를 제공합니다.
...
앱 설치 후 `sh scripts/setup_market_analysis.sh`로 분석 전용 Python 환경과 격리 검사를 준비합니다. 이후 **종목 발견**(`/discover`)에서 전략 도구를 골라 실행하거나 질문합니다. 자연어 CodeAct에는 인증된 Claude CLI가 필요하며, 선택한 전략의 직접 실행은 모델 호출 없이 같은 샌드박스와 독립 재계산을 사용합니다.

전략은 최대 12개를 AND로 조합하고 기간·기준값·순위 상위 개수를 조절합니다. 50개 계산 도구 중 현재 일봉 자료로 46개를 지원하고, 10분봉 3개와 실제 거래대금 순위는 추가 자료가 필요합니다. 매물대는 일봉으로 추정하며, HTS별로 다른 이름의 계산식도 화면에서 확인할 수 있습니다. [전략별 정의](docs/specs/market-strategies.md).

기업 조사 버튼은 발견 맥락을 보존하고 누락된 재무·최근 공시를 준비한 뒤 근거 조사를 시작합니다. 기존 검색·조사·판단 이력은 유지하고, 상세 도구·이력은 보조 패널에서 엽니다. 새로고침은 수집이나 조사를 다시 실행하지 않습니다.

시장 분석의 **전략 비교**(`/analysis/backtests`)에서는 단순 보유·이평 교차·고가 돌파·모멘텀·복합 조건 등 7개 전략을 같은 기간과 비용으로 비교합니다. 조건을 먼저 저장하고 판단 다음 거래일 시가에 체결하며, 개발/평가 구간의 자산 곡선·비용·현금·미체결과 전체 CSV를 제공합니다. 모델 호출 없이 고정 계산을 격리 실행하고 원장을 별도로 검산합니다. 현재 종목 중심 자료와 짧은 기간에 따른 잠정 연구 결과이며, [실험 계약과 첫 실행 기록](docs/specs/market-backtest.md)을 함께 확인하세요.

원본 DB는 고정 exporter만 읽고, 생성된 코드는 별도 스냅샷에서 계산합니다. 수정주가 여부·거래일 출처·이력이 불충분한 결과는 잠정 또는 미평가로 표시합니다. 지원 조건과 실행 제한은 [시장 분석 계약](docs/specs/market-codeact.md)을 참고하세요.

누락 일봉과 장기 이력을 수집하려면 다음을 실행합니다. 원본 가격·시총을 보존하면서 네이버 가격 이력을 별도 `backend/db/market_history.sqlite`에 게시합니다. 분석은 종목별로 이 이력을 우선 사용하므로 서로 다른 가격 조정 기준을 날짜별로 섞지 않습니다.

```bash
.venv/bin/python scripts/collect_market_history.py --workdir logs/market-history-20260919 --collect --publish
```

같은 작업 폴더로 중단된 수집을 재개할 수 있습니다. 이후 날짜에 갱신할 때는 새 작업 폴더를 지정합니다. 기본 범위는 원본의 최신 시세일을 끝으로 최근 2년이며, 더 오래된 가격 오류가 있으면 그 날짜까지 가져옵니다. 수집 이력이 오래됐으면 분석 기준일과 갱신 필요 여부를 화면에 표시합니다.
...
설계 의도가 궁금하면 **설계 철학**을, 코드가 어떻게 동작하는지 알고 싶으면 **구현 구조와 기술 선택**을 먼저 읽어보세요. 구현 문서는 수집·피드·스터디·AI 대화의 처리 흐름을 저장 테이블과 코드에 연결하고, 아직 구현하지 않은 범위도 구분합니다.

- [시스템 현황](docs/SYSTEM.md): 현재 구조와 운영 방법
- [기능별 스펙](docs/specs/README.md): 구현 범위와 진행 중인 설계
- [기술적 분석·추세추종 조사](docs/research/technical-analysis-trend-following.html): 서적·논문·GitHub·공개 자료 39개와 Explorer 적용 방향
- [디자인 시스템](docs/DESIGN_SYSTEM.md): 레이아웃·타이포그래피·컴포넌트 규칙
- [설계 철학](docs/PHILOSOPHY.md): 자료·해석·기대·사용자 기록을 다루는 원칙
- [구현 구조와 기술 선택](docs/TECH_DECISIONS.md): 주요 기능의 실행 흐름·저장 구조·선택 이유와 한계
