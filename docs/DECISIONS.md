...
## D-196 · 2026-09-21 · 그린 차트 구조는 가장 가까운 카탈로그 조건으로 근사해 감시·검색에 넘긴다; 해석은 규칙 우선, 모델은 종목 이름 보조만

**결정**: 차트 구조(D-195)의 "감시 규칙으로"·"이 조건으로 종목 찾기"는 그린 선 자체를 규칙으로 저장하지 않고, 카탈로그의 가장 가까운 조건(채널 → `channel_break_up/down`, 추세선 → `trendline_break_*`/`trendline_support_hold`, 레벨 → `resistance_break`/`support_break`)에 그린 구조의 매개변수(스윙 폭·점 수·기간)를 넣어 넘긴다. 차이("카탈로그는 최근 스윙으로 선을 다시 적합")는 화면에 그대로 적는다. 질문 해석은 규칙이 먼저이고, 규칙이 종목을 못 찾을 때만 haiku 1콜로 회사 이름 후보를 뽑아 같은 규칙으로 해소한다.

**맥락·이유**: 감시 규칙(D-185)과 검색은 카탈로그 조건(`strategy_id`+`params`)만 받고, 매일 평가·독립 재계산·검증 테스트가 그 위에 서 있다. 그린 선을 "기준일·값·기울기"로 고정한 별도 규칙 종류를 만들면 평가기·검증기·화면·저장 전략 모두에 새 분기가 생기고, 선은 시간이 지나면 새 스윙으로 다시 그려야 하므로 고정선 알림은 며칠 뒤 의미가 흐려진다. 카탈로그 근사는 새 분기 없이 기존 검증을 그대로 상속하고, 사용자가 원하는 것("이 채널 상단을 넘으면")을 매개변수 수준에서 유지한다. 모델 보조를 종목 이름에만 한정한 이유는 구조·기간·크기 단서는 규칙으로 충분하고(테스트 13개), 비용·지연(10~18초)을 규칙 실패 경로에만 두기 위해서다.

**기각한 대안**: (1) 고정선 교차 규칙 신설 — 위 분기 비용, 며칠 뒤 선의 유효성 문제. (2) 해석 전체를 모델에 — 매 질문 10초·$0.07, 규칙으로 되는 일. (3) 조건 변환을 숨기고 "알림 설정됨"만 보이기 — 근사임을 숨기면 사용자가 그린 선과 다른 신호를 받고 이유를 모른다(정직한 보고 원칙).

**참조**: `backend/pipeline/chart_structure.py` `to_conditions`/`interpret`/`model_assist`, `frontend/src/hooks/useGroups.ts` `useAppendMemberRule`, `structure/ChartStructureCard.tsx`, docs/specs/chart-structure.md §그린 구조 → 조건.

## D-195 · 2026-09-21 · "종목 하나에 구조를 그려줘"는 조건 검색이 아니라 결정적 그리기로 받는다

**결정**: 종목 발견의 질문이 **종목 해소 + 구조 단어(채널·추세선·고점/저점 연결·지지/저항선) + 그리기 동사(그려·표시·보여) − 검색 동사(찾아·검색·골라·추려·종목들·조건)** 규칙에 맞으면 CodeAct 실행을 만들지 않고 `pipeline/chart_structure`가 스윙 점을 찾아 채널/추세선을 적합해 차트 카드로 돌려준다. 채널 상단의 기본 적합은 **가장 높은 두 고점 연결**(회귀는 옵션), 하단은 평행선을 기간 최저가에 닿게 내린다. 스윙 폭에서 기준점이 2개 미만이면 폭을 줄여 그리고 그 사실을 표시한다. 기업 페이지 기술적 분석 패널도 같은 카드를 연다. 모델 호출 0.

**맥락·이유**: "가온전선에 대해서, 올해 전고점들을 기반으로 큰 채널을 그려줘"가 조건 검색으로 해석되어 후보 0개·"그리기는 카탈로그 조건이 아님"으로 끝났다(2026-09-21 실사용). 요청은 종목 하나의 시각화이고, 필요한 계산(스윙 검출·직선 적합)은 가격 구조 전략(D-187)에 이미 있다. 사람이 손으로 긋는 전고점 연결과 가장 비슷한 것은 두 최고점 연결이라 기본값으로 삼았다. 규칙 기반 해석을 택한 이유는 판별에 필요한 단서가 몇 개의 단어이고, 오탐이 나면 조건 검색으로 흘러가 기존 경로가 처리하므로 안전하기 때문이다(모델 해석 콜 10~18초를 아낀다).

**기각한 대안**: (1) CodeAct 모델이 샌드박스에서 PNG를 그리기 — 정적 이미지, 컨트롤 불가, 모델 콜 2~3개. (2) 카탈로그 조건(channel_break_up)의 evidence를 그대로 쓰기 — 조건의 lookback·points 매개변수가 "올해 전고점 전체"를 표현하지 못하고 판정이 fail이면 선이 안 나온다. (3) 해석을 모델에 맡기기 — 규칙으로 충분하고 10초·$0.07가 든다. 애매한 문장은 P1에서 모델 보조로 확장.

**참조**: docs/specs/chart-structure.md, `backend/pipeline/chart_structure.py`, `routers/chart_structure.py`, `frontend/src/components/structure/ChartStructureCard.tsx`, `analysis/MarketAnalysisPage.tsx`(mode=draw), `company/TechnicalScan.tsx`.

## D-194 · 2026-09-21 · 독립 재계산까지 통과한 결과는 입력 4요소가 같으면 재사용한다

**결정**: `(스냅샷 파일 해시, 스킬 파일 해시, 정규화 spec, 미지원 조건 목록)`의 해시를 키로 검증 완료 결과를 `logs/market-analysis/control/results-index.json`에 등록하고, 새 실행이 같은 키를 만나면 코드 콜·샌드박스 계산·독립 재계산을 건너뛰어 그 결과를 복사한다. `verification.cached_from`에 원 실행 id를 남기고, `status: matched`는 유지한다(후속 검색이 부모 조건으로 요구).

**맥락·이유**: 계산은 결정적이다. 같은 스냅샷(해시로 동일 증명)·같은 스킬 코드(해시)·같은 조건이면 `analytics.screen`의 출력은 바이트까지 같다. D-176의 "서버가 독립 계산한 것만 결과"라는 원칙은 원 실행에서 이미 충족됐고, 캐시는 그 증명을 상속한다. 스냅샷 해시 검증은 캐시 경로에서도 한다. 실측: 같은 spec 선택 재실행 8.3s → 0.3s, 같은 질문 자연어 43.6s → 16.7s(해석 콜만 남음).

**기각한 대안**: (1) 질문 문자열을 키로 — 모델 해석이 달라질 수 있어 같은 질문이 다른 조건일 수 있다. spec이 키여야 한다. (2) 캐시 적중 시 독립 재계산만 다시 수행 — 4초를 아끼지 못하고 얻는 것도 없다(같은 입력의 같은 함수). (3) TTL 만료 — 스냅샷 해시가 키에 들어 있어 자료가 바뀌면 키가 바뀐다. 시간 만료는 불필요.

**참조**: `runner.py` `_result_key`/`_register_result`/`_finish_from_cache`, docs/specs/market-discovery-performance.md §6.3.
...
**맥락·이유**: 종목 발견 성능 진단(docs/specs/market-discovery-performance.md)에서 정상 실행마다 모델 콜 3개 중 하나가 "끝났다"고 말하기 위한 왕복이었다(~10초, 비용의 1/3). D-176이 완료 판단을 모델에 맡긴 이유는 범용 CodeAct에서 호스트가 결과 완성 여부를 모르기 때문인데, 이 프로젝트의 결과는 항상 `analytics.screen`의 결정적 JSON이라 완성 여부를 호스트가 결정적으로 안다. 무결성 장치(evidence_ids가 성공한 관찰이어야 함, 독립 재계산 일치, 스냅샷 해시 불변)는 그대로 통과해야 하므로 신뢰 경계는 바뀌지 않는다. 선택 실행(explicit spec)은 이미 같은 경로를 쓰고 있었다. 실측: calculation 14.6s → 4.4s, 콜 3 → 2, 후보 동일.

**기각한 대안**: (1) 모델에게 `run_python`과 `finish`를 한 응답에 내게 하기 — 실행 전에 완료를 선언하는 셈이라 evidence 원칙 위배. (2) finish 콜을 haiku로 — 왕복 자체가 낭비라 모델을 바꿔도 4~5초 남음. (3) 모델의 `summary` 문장을 보존하기 위해 유지 — result에도 화면에도 쓰이지 않는 값이었다.

**참조**: `backend/pipeline/market_analysis/runner.py` `_host_finish`, 테스트 `tests/test_market_analysis_harness.py`, docs/specs/market-discovery-performance.md §6.1. D-176의 루프 계약 중 "완료는 모델이 선언" 부분을 이 항목이 대체한다.

## D-191 · 2026-09-21 · 기술적 분석의 AI 해설은 차트가 아니라 스캔 판정만 읽고, 근거 없는 문장은 호스트가 버린다

**결정**: 스캔 패널(D-190)에 'AI 해설'을 붙인다. 모델에는 차트·시세 원본이 아니라 호스트가 결정적으로 계산한 스캔 결과(성립 신호·상태·미평가)와 가격 위치 통계(20/60/120일 수익률, 52주 고저 대비, 거래량 배수)만 준다. 출력은 요약·읽기·볼 것·못 보는 것으로 나누고, 읽기·볼 것의 모든 문장은 근거 id(조건 id 또는 price 통계 키)를 달아야 하며 유효한 근거가 없는 문장은 서버가 제거한다. 판단 문장(매수·매도·전망)은 프롬프트로 금지하고 화면에는 "위 판정만 읽은 모델 의견(가설)"을 붙인다. 버튼 클릭에만 생성(도구 없는 1콜, sonnet), 같은 종목·기준일·범위는 24시간 재사용.

**맥락·이유**: 사용자 질문은 "해설이 가능한가, 부정확하려나". 부정확함의 원인은 모델이 *차트를 읽는* 데서 온다. 판정은 이미 우리가 계산했으므로 모델의 일은 사실들을 서로 연결해 읽는 것(시장 인식론 — 답 생산이 아니라 조준, D-095)이고, 근거 id 강제와 호스트 검증으로 "말한 것마다 어느 판정에서 왔는지"를 사용자가 확인할 수 있게 한다. 실측에서 모델이 갭 신호의 value를 종가로 잘못 읽은 사례가 나와 value·reference 인용을 label이 분명할 때로 제한했다 — 사실 오독은 근거 검증으로도 못 잡는 부분이라 입력 명료화로 막는다. 비용은 클릭당 약 $0.06(D-072의 활성 게이트 원칙).

**기각한 대안**: (1) 차트 이미지·시세 표를 모델에 주고 자유 해설 — 패턴 환각의 원천, 검증 불가. (2) 스캔 로드 시 자동 생성 — 페이지 열 때마다 비용, 대부분 읽지 않음. (3) 자유 서술 한 덩어리 — 어떤 문장이 어떤 판정에서 왔는지 추적 불가라 근거 칩을 붙일 수 없음. (4) 방향 점수/종합 판단 출력 — 신호 성립은 규칙 만족일 뿐이라 한 방향으로 합치는 순간 추천이 된다(D-181의 "하나의 매수 점수로 합치지 않는다"와 동일 원칙).

**참조**: `backend/pipeline/technical_commentary.py`, `routers/technical_scan.py`, `frontend/src/components/company/TechnicalScan.tsx`(Commentary), docs/specs/market-strategies.md §AI 해설, 테이블 `technical_commentaries`.

## D-190 · 2026-09-21 · 기업 페이지의 '기술적 분석'은 카탈로그 전 조건을 종목 하나에 돌리는 결정적 스캔

**결정**: 가격 카드에 '기술적 분석' 버튼을 두고, 일봉 카탈로그 51개(순위·10분봉 제외)를 그 종목 시세에 기본 매개변수로 전부 돌린다. 결과는 상태(기준일 당일 판정: 정배열·저점 높이기 등)와 사건 신호(최근 N거래일, 기본 5)와 평가 못 함으로 나누고, 화면은 드문 신호(구조·52주/연중 극값·20/60 교차·추세전환 확인·MACD 0선·매물대 60일)를 '특이점'으로 앞세운다. 특이점이 없으면 없다고 쓴다. 차트에는 특이점 마커·스윙 점·구조선만 올린다. 모델 호출 없음, 국내·미국 공통.

**맥락·이유**: 사용자 — "기업 페이지 차트에 기술적 분석 버튼 하나. 우리가 가진 전략 스킬로 그 차트를 분석. 특이점이 없을 수도 있고 있을 수도." 카탈로그가 종목 단위 순수 계산기(D-178·D-187)라 새 계산은 필요 없고 분류·표시만 정하면 됐다. 20거래일 스캔에서 통과 31개가 나와 전부 차트에 올리면 겹쳐 읽을 수 없었다 → 드문 것만 특이점, 잦은 것(갭·5일선 교차)은 접힌 목록.

**기각한 대안**: ① 모델이 차트를 읽고 해설 — 재현성 없음·비용(D-072); 판정은 결정적으로, 해설은 조사에서. ② 모든 통과 조건을 동일 비중으로 나열 — 신호 잡음이 특이점을 가림. ③ 상태 조건도 N거래일 범위로 — "최근 5일 중 하루 정배열"은 상태의 뜻이 아님.

**참조**: backend/pipeline/technical_scan.py · routers/technical_scan.py · tests/test_technical_scan.py · frontend/src/components/company/TechnicalScan.tsx · CompanyPriceResearch.tsx · docs/specs/market-strategies.md §기술적 분석 스캔 · D-178 · D-187

---

## D-189 · 2026-09-20 · 옴니바는 서버 통합 검색 위에서 순위·의도·행동을 갖는 작업 시작점으로

**결정**: 옴니바 검색을 프론트의 cmdk 필터에서 서버 `GET /api/spine/search`로 옮긴다. 순위는 정확 › 이름 접두 › 코드 접두 › 포함, 동순위 시가총액. 결과 범위는 국내 종목·미국 종목(자료가 있는 티커만)·인물/테마·종목 묶음·스터디 프로젝트. 입력 형태로 의도를 판별해 배지로 보이고, 문장은 "찾아줘"류면 종목 발견, 질문형이면 대화를 첫 항목으로 둔다. 빈 상태는 최근 본 종목·오늘 켜진 감시 신호·핀 이동 6개. 종목 행 Tab으로 그 종목의 행동(개요·실적·자료·조사·묶음 추가)을 펼친다.

**맥락·이유**: 사용자 — "검색 기능이 살짝 불편하네. 개선안 좀." 실측: "삼성" → 삼성수산·삼성디지털이미징·삼성물산·삼성전기·삼성출판사(삼성전자 없음), "NVDA" → 0건, 빈 상태 이동 17개, 문장 입력 시 같은 인용문 5줄. 원인은 이름 LIKE에 시총 가중이 없고 화면이 앞 5개만 자르는 것, 국내 표만 보는 것. 옴니바는 "두 원시 요소를 잇는 유일한 입력"(D-056 계열)인데 이동 메뉴로 퇴화했다. 서버 순위·그룹화로 옮기면 종목 묶음·기업 개요·스터디 같은 새 기능도 같은 입력에서 닿는다. 모델 호출은 없다.

**기각한 대안**: ① cmdk 클라이언트 필터 유지 + 결과 수만 늘리기 — 순위 문제(삼성전자 누락)를 못 풀고 미국 종목도 못 봄. ② 미국 전 종목 티커 표 신규 수집 — 자료 없는 종목을 열어봐야 빈 도시에, 자료 있는 티커만으로 시작. ③ 문장 기본 행동을 항상 대화로 — 사용자의 주 사용(종목 찾기)과 어긋남, 어미로 절충. ④ 초성·별칭 검색 즉시 포함 — 별칭 표가 필요해 P2.

**참조**: backend/routers/spine_search.py · tests/test_spine_search.py · frontend/src/components/shared/Omnibar.tsx · lib/recentStocks.ts · docs/specs/dock-navigation.md §옴니바 재설계 · 개선안 https://claude.ai/artifact/XWveqGsQGsU3CpoT2TUjLW · D-136 · D-185
...
## D-187 · 2026-09-20 · 차트 구조 판단(스윙·추세선·채널·돌파 후 지지)을 결정적 카탈로그 조건으로 추가

**결정**: 전략 카탈로그에 "가격 구조" 10개를 추가한다(저점 높이기·고점 낮추기·전고점 돌파·전저점 이탈·돌파 후 지지·재돌파·추세선 돌파/이탈/지지·채널 상단 돌파/하단 이탈). 피벗은 좌우 폭이 지나야 확정되고 판정일에 확정된 피벗만 쓴다(미래 참조 금지). 선은 스윙 점 2개를 잇고 3개 이상은 최소제곱, 채널은 스윙 평행선을 기본으로 회귀 밴드를 옵션으로 둔다. 기계 정밀도 안의 동률은 돌파가 아니다. evidence에 스윙 점과 선 끝점을 담아 후보 차트에 그린다.

**맥락·이유**: 사용자 — "전고점·전저점 연결해 선 긋고 돌파 후 지지, 채널 돌파 같은 걸 보는데 기능에 없다." 다듬기 패널이 '저점 높이기'·'돌파 후 지지 재돌파'를 평가 불가로 걸러낸 사례. 기존 구조 인식은 역헤드앤숄더 하나였다. 카탈로그가 단일 원본이므로 여기 넣으면 발견 검색·후속·저장 전략·백테스트·종목 묶음 감시·AI 다듬기가 자동으로 쓴다. 모델 호출 없이 결정적으로 계산된다(종목당 3ms).

**기각한 대안**: ① 모델이 차트 이미지를 보고 판단 — 비용·재현성 없음, 검증 불가. ② 피벗을 판정일 기준 미확정 상태로도 사용 — 미래 봉을 보는 룩어헤드가 되어 백테스트가 오염됨. ③ 회귀 채널만 제공 — 사람이 그리는 평행 채널과 직관이 다름, 옵션으로만. ④ 스윙 3개 이상에서도 두 점만 잇기 — 어느 두 점인지 임의성이 생겨 최소제곱으로 통일.

**참조**: backend/pipeline/market_analysis/strategies.py(`_build_structure`, `_structure_at`, `_pivot_indexes`, `_fit_line`, `_cross_exact`) · analytics.py(`chart_data` lines) · tests/test_market_strategies.py(`PriceStructureTests`) · docs/specs/market-strategies.md §가격 구조 · D-178

---

## D-186 · 2026-09-20 · 검색 질문은 버튼으로 한 번 다듬고, 다듬은 문장을 다시 해석해 실행한다

**결정**: 종목 발견 입력창에 "AI와 다듬기"를 둔다. 모델 1콜로 재진술·카탈로그 조건(두루뭉술한 표현은 대표 해석 + 대안 2~3개)·평가 불가 조건·확인 항목을 만들고, 시간축(`within_days`)과 시총 하한은 질문에 없으면 항상 선택지를 붙인다. 사용자가 고른 결과는 **문장으로 입력창에 들어가고**, 검색은 기존 해석 경로를 다시 탄다. 다듬기는 버튼을 눌렀을 때만 호출한다.

**맥락·이유**: 사용자 — "시간축·세부 조건이 애매하게 쿼리되지 않도록 AI와 함께 다듬는 기능", "전문가가 아니면 '바닥 찍고 올라오는 거'처럼 물어도 전략으로 풀어줘야 한다". 기존 실행은 해석을 실행 도중에 하고 세 항목만 되물어 나머지 기본값이 조용히 채워졌다(D-182 §12). 실측 결과 모델이 두루뭉술한 구절을 카탈로그 전략과 초보자용 대안 설명으로 잘 풀었고($0.09), 호스트 검증으로 존재하지 않는 전략은 걸러진다.

**기각한 대안**: ① 확정된 구조화 조건으로 바로 실행(모델 재해석 0, 흔들림 0) — 사용자가 문장을 더 고치는 여지를 원해 기각. 문장 경로를 택하되 조립 규칙을 서버·클라이언트가 공유해 예측 가능하게 함. ② 검색 버튼마다 자동 다듬기 — 검색당 비용 증가, 사용자가 버튼만 원함(D-072). ③ 시장(KOSPI/KOSDAQ)도 항상 확인 — 애매할 때만으로 축소.

**참조**: backend/pipeline/market_analysis/refine.py · model.py(`structured_call`) · routers/analysis.py(`/refine`) · frontend/src/components/analysis/QuestionRefiner.tsx · docs/specs/market-codeact.md §13 · D-182 · D-184

---

## D-185 · 2026-09-19 · 포트폴리오와 관심 종목을 '묶음' 하나로 통일하고 종목별 기술적 조건을 매일 결정적으로 평가

**결정**: 새 객체 `stock_groups`(kind watch|portfolio) 하나에 멤버·규칙·평가를 붙인다. 규칙은 묶음 기본 + 종목별(같은 전략은 종목 우선)이며 조건 형식은 전략 카탈로그 계약(D-180)을 그대로 쓴다. 평가는 launchd 평일 16:40에 `evaluate_strategy`로만 하고(모델 호출 0), (규칙, 종목, 기준일)마다 한 번 저장한다. 신호는 전날 pass가 아니었다가 pass로 바뀐 전이만이다. 레거시 `watchlist` 15건은 확신도·목표가·논지를 유지한 채 기본 묶음 '관심 종목'으로 1회 이관한다. 수량·매수가 입력은 P1로 두되 스키마와 손익 계산은 미리 넣었다. 조건 편집은 카탈로그 폼이 기본이고, 발견 화면의 저장 전략에서 '이 조건으로 감시'로 복사해 올 수 있다. 알림은 화면(관심목록 › 종목 묶음)까지이며 브리핑·텔레그램 연결은 P1.

**맥락·이유**: 사용자 — "종목 워치리스트가 사실상 무용지물. 포트폴리오를 구성해놓고 매일 기술적 전략을 돌려 시그널이 뜨는지 보고 싶다. 종목마다 다른 조건도 걸어야 한다. 별도 포트폴리오를 만들면 파편화가 걱정된다." 조사 결과 `follows`는 인물 12건·기업 0건, `watchlist`는 15건, 유니버스는 섹터 커버리지 단위였다. 기업 팔로우가 비어 있어 새 묶음이 그 자리를 그대로 차지해도 파편화가 없다. 전략 계산기 50개·저장 전략·`signals`·launchd 체인이 이미 있어 새로 짓는 층은 "묶음 × 조건을 매일 돌려 결과를 쌓는" 부분만이다. 매일 도는 반복 프로세스라 LLM을 끼우지 않는다(D-072 비용 의식).

**기각한 대안**: ① `follows`(entity) 재사용 — 엔티티 단위라 수량·조건이 어색하고 인물과 섞임. ② 포트폴리오·관심 그룹을 별도 객체로 — 파편화. ③ 묶음 공통 조건만 — 사용자 요구와 반대. ④ 매일 LLM 해설 생성 — 비용, 판정은 결정적으로 끝내고 해설은 발견/조사에서 요청 시. ⑤ 자연어→조건 변환을 편집 기본으로 — 편집마다 모델 비용.

**참조**: backend/pipeline/watch_rules.py · backend/routers/spine_groups.py · backend/models/groups.py · scripts/evaluate_watch_rules.py · scripts/install_launchd.py(`watch`) · frontend/src/components/follow/StocksPage.tsx · GroupPicker.tsx · hooks/useGroups.ts · docs/specs/portfolio-watch.md · D-180 · D-182

---

## D-184 · 2026-09-19 · 후속 검색은 발견 화면 안의 스레드로 쌓고, 대화(/chat)와는 통합하지 않는다

**결정**: 종목 발견의 후속 검색을 새 화면으로 대체하지 않고 같은 화면에 턴으로 쌓는다. 서버는 `GET /runs/:id/thread`로 뿌리와 모든 후속을 생성순으로 돌려주고, 화면은 `run=`이 가리키는 턴만 펼치고 나머지는 한 줄로 접는다. 후속 입력창은 스레드 맨 아래(대화의 컴포저 위치)에 둔다. 대화 화면과의 기능 통합은 하지 않는다.

**맥락·이유**: 사용자가 후속 질문 직후 이전 결과가 사라지고 스켈레톤만 남는 것을 "당황스럽다"고 했고, 대화형 UI의 친숙함을 들어 `/chat` 통합을 물었다. 데이터는 이미 스레드였다(`lineage.parent_run_id`, 조건 변경분 저장). 문제는 화면이 실행 하나만 보여준 것이다. `/chat` 통합을 택하지 않은 이유: ① D-182가 며칠 전 발견을 `/chat?mode=analysis`에서 독립 진입점으로 꺼냈고, 불편의 원인은 "대화 안에 없어서"가 아니다. ② 대화는 문서 검색+스트리밍 마크다운+인용 칩 하네스이고, 발견은 결정적 스크리닝+후보 표+차트+조건 상세+저장 전략+기업 조사 연결이다. 768px 말풍선 컬럼에 표·차트 병치가 들어가지 않는다. ③ 발견의 조건 직접 구성·저장 전략 실행은 모델 비용 0인데 대화는 매 턴 모델을 부른다. 합치면 이 구분이 흐려진다(D-072 비용 의식). 대신 대화의 상호작용 모델(턴 스택·하단 컴포저·스레드 목록)만 가져온다.

**기각한 대안**: ① `/chat`에 발견을 다시 합치기(D-182 번복) — 위 ①~③. ② 후속을 같은 run에 덧쓰기 — 이전 결과·검증 스냅샷 보존 원칙(D-181) 위반. ③ 이전 검색 Sheet로 충분 — 사용자는 맥락이 화면에 남기를 원했다.

**참조**: backend/pipeline/market_analysis/store.py(`RunStore.thread`) · backend/routers/analysis.py · frontend/src/hooks/useMarketAnalysis.ts(`useAnalysisThread`) · frontend/src/components/analysis/MarketAnalysisPage.tsx(`ThreadTurn`) · docs/specs/market-codeact.md §12 검색 스레드 · D-182

---

## D-183 · 2026-09-19 · DART finstate_all 손익계산서는 단일 분기 값이므로 차감하지 않는다 (D-002 ② 일부 번복)

**결정**: `dart_service.CUMULATIVE_STATEMENTS`를 `{"CF"}`로 줄인다. 손익계산서(IS/CIS)의 1분기·반기·3분기 `thstrm_amount`는 해당 분기 단독 금액이므로 그대로 쓰고, 4분기만 연간 − (Q1+Q2+Q3)로 만든다(하나라도 없으면 null, 0이나 가공값으로 채우지 않는다). 현금흐름표는 누적이므로 기존대로 차감하고, 재무상태표는 시점값 그대로다.

**맥락·이유**: D-002는 손익계산서·현금흐름표를 모두 누적으로 가정했다. 실데이터가 반증한다 — 삼성전자 2024 CFS: 매출 Q1 71.9조·반기 74.1조·3분기 79.1조·연간 300.9조(반기가 누적이면 146조여야 함), 영업활동현금흐름 Q1 11.9조·반기 28.8조·3분기 51.0조·연간 73.0조(누적). HPSP 2024 OFS 매출도 반기 273억 < Q1 378억. OpenDART `fnlttSinglAcntAll` 명세도 IS/CIS에만 `thstrm_add_amount`(당기누적)를 따로 두고 `thstrm_amount`는 당기 3개월이다. 기존 로직은 2분기를 (2분기 단독 − 1분기)로 계산해 2·3분기 손익이 왜곡돼 있었다. 저장 행은 원본 그대로라 조회 시 계산만 바뀌고 마이그레이션은 없다. 이 변경은 2026-09-19 Codex 세션이 발견·조사 UX 작업(D-182) 중 코드에만 반영했고, 문서 없이 들어간 것을 Claude가 실데이터로 검증해 기록한다.

**기각한 대안**: ① `thstrm_add_amount`를 추가 저장해 차감 — 컬럼·수집 변경이 필요하고 단일 분기 값이 이미 있어 불필요. ② D-002 규칙 유지 — 실데이터와 불일치.

**참조**: backend/services/dart_service.py(`CUMULATIVE_STATEMENTS`, `_build_financial_response`) · CLAUDE.md 백엔드 DART 규칙 · D-002

---

## D-182 · 2026-09-19 · 종목 발견을 독립 진입점으로 만들고 조사 실행에 필요한 자료 준비를 포함

**결정**: 사용자가 승인한 UX 방향에 따라 `/discover`를 자연어 종목 발견의 진입점으로 승격한다. 기존 시장 분석 URL과 검색·전략·조사·사용자 판단 이력은 유지한다. 입력→후보 탐색→기업 조사→후속 관찰 순서로 화면을 구성하고, 도구 목록·설정·이력·실행 로그는 보조 동선으로 옮긴다. 기업 탭과 복귀 링크는 발견 맥락을 보존한다. D-057의 탐색 해체 이후 추가된 CodeAct 종목 발견에 한해 독립 진입점을 제공한다.

**자료 준비**: D-181의 GET 조회 전용·모델 권한 제한은 유지한다. 사용자가 기업 조사 POST를 실행하면 호스트가 선택 기업의 부족한 재무·최근 공시를 제한적으로 준비한 뒤 읽기 전용 근거 묶음을 만든다. CodeAct에는 수집기·네트워크·운영 DB 쓰기 권한을 주지 않는다. 페이지 진입/새로고침은 실행을 만들지 않으며 첫 조사 요청은 발견 생성과 연결한다. 성공·명시적 자료 없음·오류를 구분하고 이력에 준비 상태를 보존한다.

**맥락·이유**: 도구 목록과 설정이 자연어 입력·후보·조사 결과보다 먼저 나와 사용 흐름이 길어졌다. 저장된 자료만 읽는 기존 조사로는 아직 수집하지 않은 재무자료가 지속적인 빈 결과로 남는다. 사용자의 조사 행동을 해당 기업에 필요한 표준 자료 수집의 승인으로 해석하되, 공개일/수집일과 과거 기준일의 의미를 보존한다.

**기각한 대안**: 모든 종목 자동 수집, GET/렌더에서 새 조사 실행, 중복 확인창, 생성 코드에 원본 쓰기 허용, 기존 결과/사용자 기록 덮어쓰기, 자료 없음과 API 오류를 같은 캐시에 저장, 신뢰되지 않은 HS 연결과 콜 수집을 자동 확증에 사용.

**참조**: [실행 계획·화면 상태·계약](specs/market-codeact.md#12-발견조사-ux-개편과-필요한-자료-준비-2026-09-19). 사용자 승인 2026-09-19. 구현과 검증 상태는 해당 스펙을 따른다.

---

## D-181 · 2026-09-19 · 발견 이력은 원본 밖에 보존하고 제한된 근거 조사와 사용자 판단을 연결

**결정**: 승인한 D-180 기획을 실제 시장 분석과 기존 국내 기업 화면에 연결한다. 전략 버전·발견 맥락·사용자 판단·질문별 조사 묶음은 `logs/market-discovery/discovery.sqlite`에 저장한다. 기술적 후속 검색은 이전 조건의 명시적 변경분과 스냅샷/후보 범위를 유지한다. 조사 모델에는 호스트가 읽기 전용으로 확보한 발췌와 수치만 전달하고 조회 도구·운영 DB 권한은 주지 않는다.

**맥락·이유**: 원본 보호 요구를 유지하면서도 개인 사용자의 판단 이력은 기록해야 한다. 기존 콜 날짜와 HS/기업 매핑, 질문 종합 판정에는 그대로 확증에 사용하기 어려운 부분이 있어 운영 파이프라인의 자동 확증을 재사용하지 않는다. 사용자 메모, 원문 작성자의 주장, 저장 수치와 모델의 추론을 구분하고 공개일·귀속이 불확실한 자료는 과거 시점 판단에서 제외하거나 한계를 표시한다.

**계약**: 독립 검증된 실행만 전략/발견으로 저장하며 미지원 요청 조건을 조용히 제거한 전략은 저장하지 않는다. 수정은 새 버전이며 멱등 재시도와 동시 수정 충돌을 처리한다. 조사 버튼을 누를 때 실행하고 취소·실패·중단 후에도 근거를 보존한다. 후속 비교는 이번에 읽은 자료 묶음의 변화이며 전체 뉴스 감시나 원문 삭제 판정이 아니다. 추천은 목적별 지원 도구 예시와 선택 종목의 실제 통과 조건으로 한정한다.

**기각한 대안**: 원본 DB에 생성 코드 쓰기 권한 부여, 조회 시 자동 연구/수집, 수집일을 공개일로 대체, 공동언급·HS 제안을 고객 관계/기업 매출로 확정, 질문 단위의 확인을 모든 인과 엣지로 전파, 모델이 사용자 가설을 덮어쓰기.

**참조**: [개발·API 계약](specs/market-codeact.md#11-승인-기획-구현-계획-2026-09-19-진행), `backend/pipeline/market_analysis/discovery_*.py`, `frontend/src/components/company/DiscoveryResearch.tsx`. 원천 공개일 정비·자동 일정 관찰·사용 이력 기반 개인화는 별도 확장 범위다.

---

## D-180 · 2026-09-19 · 자연어 종목 발견과 기존 리서치·팔로우업을 제품의 중심 흐름으로 연결

**결정**: 사용자가 여러 전략 도구와 CodeAct로 종목을 동적으로 찾고, 유용한 검색을 저장·추천받고, 관심 종목의 시장 담론·전방 산업·실적/컨퍼런스콜·수출입 자료를 기존 Explorer에서 이어서 조사하는 방향을 우선한다. 발견 이유와 시점을 사용자의 가설·후속 관찰까지 보존한다. D-179 백테스트는 신호의 특성과 실패 사례를 확인하는 보조 도구로 유지한다.

**맥락·이유**: 사용자는 지수 개발·완전 자동 시스템 트레이딩을 목표로 하지 않으며, 개인 투자자의 종목 발견과 리서치를 통한 투자 의사결정을 시스템의 강점으로 지정했다. 검색 수익률 최적화에 개발을 집중하거나 기업마다 새 보고서만 생성하면 기존 수집 자료·원문 읽기·질문·사용자 기록의 연결 가치가 약해진다.

**설계 기준**: 검색 정의와 날짜별 실행을 구분해 버전으로 보존한다. 추천은 조사 목적·관측 신호·자료 가용성에 대한 이유를 제시한다. 후속 질의는 이전 조건과 변경분을 보존한다. 생성 코드의 원본 조회/계산 경계는 유지하고 리서치 자료는 통제된 읽기 경로로 전달한다. 내러티브·HS 관련 종목 제안은 검토할 가설이며 읽은 원문·측정된 통계와 구분한다. 사용자 판단과 시스템 해석을 따로 보존한다.

**기각한 대안**: 백테스트의 단기 수익률 중심 추천, 종목 코드만 전달해 발견 맥락을 잃는 화면 이동, 기존 리서치 기능과 별개인 중복 기업 리포트, 모든 검색 후보의 자동 심층 조사, 전략 저장을 통한 활성 생성 코드의 자동 배포.

**상태·참조**: 제품 방향을 기록한 결정이며 후속 기능 구현 완료를 뜻하지 않는다. [발견·검색 연결 설계](specs/market-codeact.md#10-발견--전략-재사용--기업-리서치-후속-설계), [전략 저장·추천](specs/market-strategies.md#검색-전략-저장추천-후속-설계), [기업 리서치·팔로우업](specs/company-research.md#발견한-종목의-리서치팔로우업-후속-설계). 기존 동작·원본 DB·사용자 기록은 이번 설계 작업에서 변경하지 않는다.

---

## D-179 · 2026-09-19 · 전략 비교는 사전 고정 조건·격리 계산·별도 원장 검산으로 실행

**결정**: 기술적 분석 조사 후 첫 실험은 단순 보유를 포함한 7개 전략으로 제한한다. 같은 자료·비용으로 개발/평가 구간을 독립 실행하고, 복합 조건은 52주 돌파→역헤드앤숄더 선행→20일선 유지 순으로 추가해 비교한다. 실험 조건·입력·계산 코드의 해시를 결과보다 먼저 저장하며 조회/새로고침은 새 실행을 만들지 않는다.

**맥락·이유**: 조건 검색의 적중 종목과 매매 전략의 수익은 다른 문제다. 기존 자료는 현재 종목 중심이고 과거 시총과 상장/상폐 이력이 불완전하다. 첫 비교는 확보된 준비 이력에 맞춰 개발 2026-02-03~06-30, 평가 07-01~09-18로 고정하고 시총 필터를 끈다. 보통주 전용·생존 편향 제거·배당 포함 수익을 보증하지 않으며, 계산이 성공해도 연구용 잠정 결과로 표시한다.

**계약**: 판단일 종가/NAV로 목표 수량을 정하고 다음 관측일 시가에 차이만 거래한다. 매도 후 매수, 예시 비용, 부족 현금에 대한 비례 축소, 실패 주문·지연 평가·종료 보유를 공개한다. 후보가 적으면 현금으로 남기므로 조건 추가에 따른 노출 감소를 신호의 독립 효과로 해석하지 않는다. 고정 Python 계산을 두 샌드박스에서 재실행한 뒤 입력 해시를 다시 확인하고, 호스트가 입력 가격을 이용해 거래·현금·평가 원장을 별도로 검산한다. 원본 DB에 실험 상태를 쓰지 않는다.

**기각한 대안**: 결과를 본 뒤 기본 기간/조건 조정, 미래 결측 없는 종목만 선별, 과거 시총을 최신 값으로 대체, 같은 코드의 재실행 일치만으로 회계 정확성 판정, 임의 모델 코드/SQL을 백테스트 API로 수신. 시점 정보 누출·선택 편향·원본 접근 범위 확대 또는 검증 공백을 만든다.

**참조**: [시장 전략 비교 백테스트](specs/market-backtest.md), `backend/pipeline/market_analysis/backtest_{engine,data,runner,verification}.py`, `backend/routers/backtests.py`, `frontend/src/components/analysis/BacktestPage.tsx`. 실제 실행 `321e50b8654a44b5b45d4e9add08f509`, 검증 산출물 `logs/market-backtest-verification/`.

---

## D-178 · 2026-09-19 · 전략 카탈로그의 계산 기준을 선택 화면·CodeAct·검증기가 공유

**결정**: HTS 화면의 50개 항목을 버전이 있는 계산 카탈로그로 구현하고, 이름·기간·판정식·필요 자료를 공개한다. 자연어 해석과 직접 선택은 같은 `AnalysisSpec.strategy_conditions`를 사용하며, 선택 실행도 기존 격리 실행과 독립 재계산을 거친다. 기존 DB의 쓰기 권한은 추가하지 않는다.

**맥락·이유**: 사용자가 시세동향·지표신호·순위 도구들을 모아 조합하려고 한다. 이름이 같아도 HTS마다 매물대·추세전환·Stochastic 매수의 정의가 달라 원본 프로그램과 동일하다고 주장할 수 없다. 계산식과 기본값을 명시하고, 일봉 매물대는 추정치로 표시한다. 실제 거래대금을 종가×거래량으로 만들거나 일봉을 10분봉으로 대체하지 않는다.

**계약**: 최대 12개 AND 조건, 최근 N 관측 거래일 중 충족 여부, 기준일에 고정된 시장·시총. 순위 조건들은 일반 조건을 통과하고 선택 지표를 계산할 수 있는 동일 집합에서 독립적으로 순위를 매긴 뒤 교집합을 취하며 동률은 종목코드 순이다. 조건 탈락·자료 부족을 구분하고 조건별 수치·날짜·순위를 JSON/CSV에 보존한다. 50개 계산 도구 중 현재 자료로 46개를 제공하며, 10분봉 3개·실제 거래대금 1개는 미평가를 반환한다.

**기각한 대안**: 모델이 실행마다 지표 공식을 임의로 재작성, 이름만으로 타 HTS와 동일성을 선언, 자료가 없는 조건을 삭제해 통과 처리, 선택한 순서에 따라 순위 후보 집합을 반복 축소. 결과 재현성과 조건 보존을 해친다.

**참조**: [전략 도구 정의](specs/market-strategies.md), `backend/pipeline/market_analysis/{strategies,strategy_screen}.py`, `frontend/src/components/analysis/StrategyLibrary.tsx`.

---

## D-177 · 2026-09-19 · 시장 이력 보강은 별도 수정주가 저장소로 게시

**결정**: 수집한 NAVER/FDR 가격 이력을 별도 `market_history.sqlite`에 저장하고, 시장 분석 exporter는 수집된 종목의 가격 시계열 전체를 이 소스에서 읽는다. 기존 `stock_explorer.db`의 가격·시총·주식수·다른 사용자 자료는 수정하지 않는다. 시총과 주식수는 기준일의 원본 값으로 연결하며, 가격은 날짜별로 다른 소스를 섞지 않는다.

**맥락·이유**: 사용자가 실제 질의를 실행했으나 데이터 부족으로 평가하지 못해 수집을 요청했다. 기존 백필의 200행 기준은 중간 누락과 52주 계산에 필요한 앞부분 이력을 복구하지 못한다. 수집 표본을 대조하니 일부 종목에 분할·병합 조정으로 보이는 5배·10배 차이가 있어, 기존 정상 행을 두고 빈 날짜만 수정주가로 채우는 방법도 가짜 가격 단절을 만들 수 있었다.

**계약**: HTTP timeout·재시도·속도 제한, 재개 가능한 staging, OHLCV·중복 검증, 출처·수집 시각·범위 기록, 검증 후 원자적 게시. 저장소 파일은 읽기 전용이며 CodeAct가 갱신하지 않는다. 공식 거래일 달력은 여전히 미확인이다. 자동 수집 스케줄은 이번 작업에서 변경하지 않는다.

**기각한 대안**: 원본 전체 덮어쓰기, 원본에 보정 기준이 다른 행을 빈 날짜만 삽입, 종가만 보정해서 기존 고가·저가와 결합. 원본 보존과 동일 가격 기준을 동시에 만족하지 못한다.

**참조**: `scripts/collect_market_history.py`, `backend/pipeline/market_analysis/snapshot.py`, `tests/test_market_history_collection.py`, [시장 분석 계약](specs/market-codeact.md). 가격 소스의 조정 기준은 [FinanceDataReader 안내](https://github.com/FinanceData/FinanceDataReader/wiki/Quick-Start), 소스 선택은 [공식 README](https://github.com/FinanceData/FinanceDataReader) 참조.

---

## D-176 · 2026-09-19 · 시장 CodeAct는 불변 스냅샷·OS 격리·독립 재계산으로 원본 보호

**결정**: 기존 Explorer 시세를 고정 읽기 전용 exporter로 Parquet/manifest에 내보내고, 모델 생성 코드는 macOS 격리 환경에서 실행한다. 원본 DB·앱 소스·인증·localhost에 접근할 수 없으며 자기 임시 계산 영역만 쓴다. 모델 호출기는 내장 도구·MCP·프로젝트 훅을 차단한다. 격리를 시작할 수 없으면 실행하지 않는다.

**맥락·이유**: 사용자가 로컬 `code-as-a-tool` PoC의 자연어 시장 검색을 Explorer에 추가하되 기존 DB 삭제·덮어쓰기를 막도록 요구했다. 기존 DB 연결은 쓰기 가능하며 시세 조회도 캐시 미스 시 수집·저장할 수 있다. 조회 프롬프트나 SQL 키워드 차단으로 원본 보호를 보장할 수 없으므로 생성 코드와 원천 연결을 분리했다.

**검증·범위**: 조건을 확정하고 같은 입력/스킬 해시로 별도 샌드박스에서 재계산한 결과만 제공한다. 실행·관찰·비용·이벤트는 운영 DB 밖에 보존한다. A2UI는 해석 선택만 받으며 권한을 확장하지 않는다. 첫 패턴은 명시적인 `consecutive-pivots-v1`, 52주는 종가가 직전 365일 고가 최댓값을 초과하는 사건, MA는 사용자 기간을 보존한다. 수정주가·공식 거래일 달력·이력 공백은 미검증/미평가로 표시한다.

**기각한 대안**: 공용 DB 연결을 CodeAct에 전달, 프롬프트만으로 SELECT 제한, 쓰기 가능한 scratch 안에 실행 정책/다음 스크립트 보관, 자동 스킬 배포, 실패 시 자동 백필. 원본 변경·우회 또는 조건 완화 가능성 때문에 사용하지 않는다. Linux/VM 어댑터와 완전한 메모리 격리는 첫 버전 범위 밖이다.

**참조**: [시장 분석 계약](specs/market-codeact.md), `backend/pipeline/market_analysis/`, `backend/routers/analysis.py`, `frontend/src/components/analysis/`. 임시 원천 파괴 시도와 불변성 검사, 실제 원천 read-only 해시 대조, 실제 모델 생성 코드의 정답 fixture 대조를 수행했다.

---

## D-175 · 2026-09-18 · 읽기 의도별 하이라이터와 표시 단위 AI 대화

**결정**: 강조/이해하기/검토하기/관련 자료를 선택 동작에 연결한다. 기존 annotation kind는 보존하고 intent를 추가한다. 새 AI 작업은 `study_annotation_actions`에 불변 snapshot·버전·결과를 저장하며 단일 문서/프로젝트 공통 큐에서 실행한다. 단순 강조와 코멘트 저장에는 모델을 호출하지 않는다.

**맥락·이유**: 사용자는 개념을 이해하거나 주장을 비판적으로 검토하거나 관련 소스를 찾기 위해 하이라이팅 자체를 질문으로 쓰고자 했다. 기존 pending 거절과 프로젝트의 입력란 채우기는 연속 독서를 끊었고, 전체 대화의 마지막 답변을 자동으로 이어받으면 다른 표시의 논점이 섞인다. 표시마다 독립 대화를 만들고 명시적으로 선택한 답변의 조상만 이어가는 방식으로 해결했다. AI 실패·취소·재시도는 표시와 이전 답변을 삭제하지 않는다.

**처리·근거**: 주석 저장과 요청 접수를 한 트랜잭션으로, 동일 키·입력은 멱등 처리한다. 소속·revision·exact를 검사한 원문과 인접 본문, 사용자 코멘트를 고정한다. DB의 프로젝트/단독 문서별 FIFO와 running UNIQUE로 중복 실행을 제어하고 기존 자유 대화 큐는 유지한다. 작업 의도는 코드 값이며 Haiku가 검색 방법만 계획한다. 기존 Sonnet 합성·수집 검색·웹 도구를 재사용한다. 관련 자료는 현재 원문·동일 본문/URL 재전파를 제외하고 발췌 기준의 연결 이유를 요구한다.

**실행 중 발견한 의존성 문제**: 최초 수집 검색이 임베딩 모델 다운로드에서 수분간 대기했다. 대화형 검색은 로컬 모델 캐시만 사용하고 미가용 시 BM25 결과를 유지하도록 `search.py`·`chunks.py`를 보완했다. 리랭커도 캐시가 없으면 RRF 순서를 유지한다. 인덱스 구축이 모델을 확보하는 기존 경로는 유지한다.

**화면·소유권**: 기존 통합 노트 안에서 표시별 AI 답변을 보관한다. 사용자 코멘트와 별개이며 ‘가져오기’도 초안 추가 후 명시적 저장을 거친다. 대기 취소/중단 복구/실패 재시도를 제공하고 조회·재열기·페이지 새로고침은 모델을 실행하지 않는다.

**선택하지 않은 대안**: 프롬프트 문구의 정규식으로 버튼 의도를 재추정, 현재 체크된 모든 주석을 새 표시 요청에 섞기, 글로벌 마지막 대화 자동 연결, AI 답을 사용자 노트에 자동 저장, 요청마다 여러 에이전트를 신설하는 방식을 택하지 않았다. 관련 자료의 자동 프로젝트 추가와 서버 시작 시 자동 생성 재개도 포함하지 않는다.

**참조**: `models/study_actions.py`, `pipeline/study_actions.py`, `pipeline/study_intents.py`, `StudyPalette.tsx`, `AnnotationActions.tsx`, `useStudyActions.ts`; [현행 계약](specs/study-mode.md). 검증은 기존/신규 스터디 테스트, 타입·빌드, 단일/프로젝트 격리 브라우저 및 서비스 모델 실행(`logs/study-intents/`).
...
→ D-183에서 ②의 손익계산서 부분 번복(현금흐름표 차감·CFS 우선·계정 정규화는 유지)
