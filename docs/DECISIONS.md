...
**참조**: `scripts/run_backfill.sh`(신설) · `scripts/backfill_enrich_batch.py`(`--until`·`_deadline`·`stopped`) · `scripts/install_launchd.py`(backfill 잡) · [[D-117]](effort low — 창 안에서 처리량 결정) · [[D-106]](기상 시 놓친 회차 실행) · [[D-116]](pgrep 오탐 교훈)
