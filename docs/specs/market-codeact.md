...
- [x] 진입점: `/discover`의 ‘종목 발견’을 독립 탐색으로 제공하고 기존 `/chat?mode=analysis` 링크를 보존한다. 자연어 입력을 먼저, 추천·저장 전략은 재사용 동선, 전체 도구·이력·백테스트는 보조 동선에 둔다.
- [x] 후보 탐색: 목록과 선택 종목의 차트/충족 근거를 함께 보여준다. 선택·검색 결과는 URL에, 복귀 위치는 UI 메모리에 보존한다. 중요 미평가 정보는 결과 근처에 노출하고 상세 경고/로그는 접는다. 후속 조건 수정은 결과 위에서 접근한다.
- [x] 기업 조사: 후보의 ‘기업 조사’ 한 번으로 발견 기록과 첫 조사를 준비한다. 기존 기록 재진입·새로고침은 실행하지 않는다. 기업 화면은 결과/가격 맥락을 우선하고 설정·이력·전체 발견 조건은 접는다. 탭 이동·검색 복귀 시 발견 맥락을 유지한다. 기존 관심목록과 사용자 판단을 연결한다.
- [x] 자료 준비: 사용자 조사 POST에만 선택 기업의 필요한 재무·공시를 bounded 수집한다. 보유/최신성 확인 → 부족 자료 수집 → 검증/저장 → 읽기 전용 근거 묶음 → 모델 조사. 진행 상태는 조사 이력에 저장하며 새 실행 없이 조회한다.
- [x] 검증·문서: 실패/취소/중복/과거 기준일/재접속, 검색→조사→탭 이동→복귀, 모바일·키보드와 기존 회귀를 검증하고 SYSTEM/DECISIONS를 갱신한다.

2026-09-19 검증: HPSP(403870) 실 조사에서 재무 11개 기간·공시 84건을 준비한 뒤 모델 종합까지 완료(119초·$0.30, partial). 첫 시도는 180초 한도 초과가 'JSON 미반환'으로 보고돼 한도를 300초로 올리고 가드 종료·CLI 오류를 구분해 보고하도록 고쳤다. 백엔드 unittest(65건)·tsc·프론트 내비게이션 테스트 통과.

### 검색 스레드 (2026-09-19, D-184)

후속 검색은 새 화면으로 대체되지 않고 같은 화면에 **턴으로 쌓인다**. 스레드 = 최초 검색과 그 뿌리를 공유하는 모든 후속 검색이며, 서버 `GET /runs/:id/thread`가 뿌리부터 생성 순서로 돌려준다(부모는 `lineage.parent_run_id`, 아직 해석 전이면 요청의 `parent_run_id`). 화면은 `run=`이 가리키는 턴만 펼치고 나머지는 한 줄(상태·후보 수·질문·바뀐 조건 라벨)로 접는다. 접힌 턴을 누르면 `run=`이 바뀌고 저장된 결과가 다시 열린다(재계산 없음). 후속 입력창은 스레드 맨 아래에 있고, 제출 직후 새 턴을 클라이언트에서 먼저 붙여 이전 결과가 사라지지 않는다. 뒤 턴을 열면 그 턴으로 스크롤한다. 대화(`/chat`)와의 통합은 하지 않는다 — 이유는 D-184.
...
### 판정 범위 기본값 (2026-09-20)

첫 화면의 ‘판정 범위’ 선택은 `RunRequest.default_within_days`(1~250)로 전달되고 모델 컨텍스트에 실린다. 문장에 기간이 없는 일봉 조건의 `within_days` 기본값이며, 문장에 적힌 기간이 우선하고 순위·10분봉 조건은 1로 남는다. 기준일(as_of)은 판정하는 날, 판정 범위는 그날 포함 며칠 안의 발생을 인정하는지다.
