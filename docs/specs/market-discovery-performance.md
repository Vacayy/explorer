...
| 5 | 내보내기 행 검증 최적화(SQL 이동 대신 메모·전개) | 첫 실행 -4s | **적용** | export 6.0s → 4.1s(in-process), Parquet·manifest 동일. §6.5 |
| 6 | 해석 콜을 haiku/Jev로 A/B | -3~9s | 후보 — 정확도 A/B 데이터·자격증명 필요, 사용자 판단 대기 | |
...
### 6.5 스냅샷 내보내기 행 검증 최적화 (2026-09-21)

SQL로 검증을 옮기는 대신(코드별 품질 통계·중복일·누락일 집계가 Python 사전에 얽혀 있어 manifest 등가 증명이 어려움) 프로파일이 지목한 행별 비용만 줄였다. 모두 출력 불변.

| 지점 | 전 | 후 |
|---|---|---|
| 종목코드 정규식 + `_iso_date` 왕복 | 137만 행마다 | 검증 통과한 코드·날짜 집합에 메모(코드 2,992·거래일 1,300개가 반복) |
| `_number` | 960만 회 `float()` + 예외 처리 | `type(value) is float` 빠른 경로 |
| `_bad_prices` | 제너레이터 `any` + `max`/`min` | 같은 진리표를 비교식으로 전개 |
| 행 사전 생성 | `dict(zip(DAILY_COLUMNS, map(_number, …)))` | 필드별 명시 생성 |

등가 증명: 원 코드(git stash)와 최종 코드로 같은 원본을 내보내 `daily.parquet`·`universe.parquet` sha256과 `snapshot_id`·`exported_at`을 제외한 manifest 해시가 동일. 데이터 테스트 통과.

실측(in-process `export_snapshot`): **6.0s → 4.1s**. 스냅샷 재사용(§6.2) 덕에 이 비용은 원본이 바뀐 뒤 첫 실행에만 든다. 내보내기 코드 해시가 지문에 들어 있어 이 커밋 뒤 첫 실행은 한 번 새로 내보낸다.

## 7. 종합 (2026-09-21, 1~5 적용 후)

| 실행 유형 | 진단 시점 | 적용 후 | 남은 시간의 구성 |
|---|---|---|---|
| 자연어 실행(조건 3, 캐시 미적중) | 46.2s / $0.11 / 콜 3 | **약 24s** / $0.096 / 콜 2 (해석 ~10~18 + 스냅샷 0.2~0.8 + 코드 콜 ~9 + 계산 ~2.5 + 검증 ~2.5) | 모델 2콜이 80%. 해석 콜 편차(9~34s)가 최대 변수 |
| 자연어 실행(같은 질문·같은 spec 재실행) | 46.2s | **16.7s** / $0.072 | 해석 콜만 |
| 선택 실행(모델 0콜, 캐시 미적중) | 12~21s | **약 5.5s**(시총 하한 spec 실측 3.8s) | 지문 0.4~0.8 + 계산·검증 각 1.4~2.5 |
| 선택 실행 재실행 | 12~21s | **0.3s** | |
| 첫 실행(원본 갱신 직후, 내보내기 포함) | +7.4s | +약 5s | 하루 1회 |

목표 대비: 선택 실행 5초 이하 — 근접(표본 spec 기준 약 5.5s, 시총 하한 3.8s). 자연어 25초 이하 — 해석 콜 편차에 따라 경계. 비용 $0.07 이하 — 미달($0.096): 남은 비용의 3/4이 해석 콜(카탈로그 30K자 입력)이므로 6번(해석 콜 모델·입력 축소)이 다음 지렛대.

## 8. 다음 후보 (2026-09-21, 1~5 적용 뒤 진단)

남은 자연어 실행 약 24초의 구성: 해석 콜 10~18s($0.07) · 코드 콜 ~9s($0.02) · 계산 2.5 · 검증 2.5 · 지문 0.4~0.8. 아래는 효과 큰 순서. ★는 D-176 계약을 건드려 DECISIONS 기록이 필요한 것.

| # | 후보 | 근거 | 예상 | 위험 |
|---|---|---|---|---|
| A ★ | **카탈로그 모드에서 코드 콜 제거** — 해석 뒤 호스트가 `analytics.screen(spec)`을 직접 실행(선택 실행 경로와 동일). 실패할 때만 모델에 관찰을 넘김 | 저장된 18개 자연어 실행의 코드는 전부 이 한 줄 호출. 모델이 코드를 쓰는 콜이 정보를 더하지 않음 | -9s, -$0.02 → 자연어 ~15s / $0.075 | CodeAct 루프가 카탈로그 모드에선 "해석 1콜"이 됨. expression·pattern 모드는 유지 |
| B | **refine → spec 직접 전달** — 질문 다듬기(D-186)가 이미 조건을 만들었으면 실행 요청에 `spec`으로 보내 해석 콜 생략 | refine 결과와 해석 결과가 같은 카탈로그 조건. 지금은 같은 걸 두 번 만듦 | 다듬기 경로 자연어 실행 0콜 → ~5s | 사용자가 다듬은 문장과 spec의 불일치 표시 필요(화면) |
| C | **카탈로그를 시스템 프롬프트로 이동 + 압축** — 60개 카탈로그(30K자)가 매 콜 사용자 컨텍스트에 실림. 정적이므로 시스템 프롬프트에 두면 프롬프트 캐시 대상, `formula`·`parameters` 산문을 빼면 ~8K | 해석 콜 비용 $0.07의 대부분이 입력 토큰 | 비용 -40~60%, 지연 -2~4s | 압축으로 애매한 질문 매핑 정확도 하락 가능 — A/B 필요 |
| D | **`plan` 길이 상한 4000 → 600자** | 콜당 7~8초가 출력 생성. plan 산문이 출력의 큰 몫 | 콜당 -1~3s | 없음(plan은 화면 이벤트 표시용) |
| E | **후속 검색 scope 푸시다운** — `universe_codes`(후보 범위)를 Python에서 거르는 대신 Parquet 쿼리 `WHERE code IN (…)`로 | `strategy_screen.py:58`·`analytics.py:318`이 137만 행을 다 읽은 뒤 버림 | 후보 범위 후속 질문 계산 2.5s → 0.1s | 없음 |
| F | **해석 콜 haiku A/B** | 빈 호출 haiku 3.0s vs sonnet 4.3s, 생성도 빠름 | -3~6s, 비용 -70% | 정확도. 저장된 18개(질문 → spec)를 정답으로 비교하는 실험 $0.5 |
| G | **원본 갱신 직후 스냅샷·지문 선계산** — 16:10 ingest 잡 끝에 내보내기 1회 | 하루 첫 실행이 5s를 냄 | 첫 실행 -5s | 랩탑 스케줄 결손 시 그냥 첫 실행이 냄(현행과 동일) |
| H | **저장 전략(D-181) 결과 선계산** — 갱신 뒤 저장 전략을 한 번 돌려 결과 캐시에 넣음 | 캐시 적중 0.3s | 저장 전략 재실행 즉시 | 전략 수 × 3s 배치 비용 |
| I ★ | **호스트가 쓴 코드는 독립 재계산 생략** — A 적용 뒤 계산 코드가 호스트 것이면 두 번째 샌드박스 실행은 결정성만 확인하는 셈 | 검증 2.5s | -2.5s | D-176 "독립 재계산" 원칙. 모델이 쓴 코드일 때만 재계산으로 축소하는 결정 필요 |
| J | **스냅샷 폴더 GC** — 종료된 실행만 참조하는 오래된 스냅샷 정리 | 22개·440MB | 디스크 | 과거 실행 차트가 스냅샷을 읽음 — 보존 기간 정책 필요 |

| K | **Jev를 해석의 1차 통과로** — 질문을 Jev에 "60개 조건 각각 해당 확률 + within_days·시총 하한 추출 + 확신도"로 묻고, 확신이 높으면 spec을 만들어 0콜 실행, 낮으면 sonnet 해석으로 폴백 | 남는 유일한 큰 콜이 해석(10~18s, $0.07). Jev는 분류·추출 전용(70~500ms, 입력 $0.042/MTok)이라 정확히 이 모양 | 비다듬기 경로 해석 10~18s → ~0.5s | 얼리액세스 대기열·자격증명 정책·한국어 지원 미확인·출시 1주. 평가셋(질문→spec) 18개뿐. A 없이는 코드 콜이 남아 효과 반감 |

권장 순서: E(무위험·즉시) → D → A → B → C·F(A/B 실험) → G·H → I → J. K는 A·B 뒤, 아래 세 관문을 통과하면 실험: ① 외부 API 키 허용 여부(ANTHROPIC_API_KEY 불가 결정이 제3자 키에도 적용되는지) ② 한국어 입력 지원 ③ 저장 실행·refine 로그로 평가셋 100개 이상 확보 후 정확도·보정 측정. 통과 시 설계는 "Jev 1차 + 확신도 게이트 + sonnet 폴백"이며 단독 해석기로 두지 않는다.
