...
### 기계의 세 가지 태도 (2026-07-12 추가)

시스템의 구조적 강점: **사람들의 심리·행동을 관찰하면서 스스로는 감정·기억의
한계 없이 진단한다** — 참여자가 아니라 관찰자다 (skin-in-the-game 없는 눈).
이 강점 위에서, 수집·통합(거울)을 넘어 기계가 취할 세 가지 능동적 태도:

1. **불편한 질문** — 통념과 내 포지션에 대해 가장 불편한 질문을 주기적으로 생성.
   ACH(A-4)의 능동형: "이 논지가 틀리려면 무엇이 참이어야 하는가", "지금 아무도
   묻지 않는 질문은 무엇인가". 산출: 주간 devil's advocate 브리핑 항목.
2. **너머 보기** — 2차적 사고 (Marks의 second-level thinking): 통념이 이미
   프라이싱한 것을 명시한 뒤("시장은 X를 기대한다"), 그 너머를 묻는다 —
   "X가 실현되면 그 다음은? X를 모두가 기대한다는 사실 자체의 함의는?"
   브리프·답변에서 '통념 요약'과 '통념 너머' 블록을 분리.
3. **소외 탐지** — 주목의 부재가 신호다 (mention_surge의 쌍대). 관심의 쏠림은
   가격에 반영되고, 소외는 비효율을 낳는다 (예: stakeholder 관찰 — 지금의 코스닥).
   계량: 펀더멘털·밸류에이션 스크린 × 언급 부재의 교차 — "괜찮은데 아무도
   말하지 않는 것" 스캐너. LLM 0, 기존 데이터 재조합.
