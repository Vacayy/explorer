...
팔로우             서브탭: 팔로우(/follow: 내가 따라가는 종목·채널·블로그·태그 허브) ·
                   **유니버스**(/follow/universe: 담당 섹터 커버리지 — 산업 맵 그룹×밸류체인 단계를 기계 제안(후보)→사람 승인으로 큐레이션, UniversePage, D-037·D-039. + **'이 섹터의 내러티브' 집약 뷰**(D-074 Phase 1: 그룹을 건드리는 내러티브가 섹터 단위로 모임, 월드모델로 링크 — 섹터=팔로우×월드모델 cross-cutting 앵커)) ·
