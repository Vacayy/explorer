...
| `reports` | 앵커별 | 통합 리포트 캐시(integrated-report) — anchor_topic PK·title·body(Top-down md)·members_json·stocks_json·members_hash(구성원 topic:version, 변동 시 stale). 연쇄 LLM 결과 저장, '다시 생성'(refresh)으로만 갱신 |
...
| `report` | **통합 리포트**(integrated-report, D-041·D-042) — 앵커 주제 + `related_narratives`(공유 인과 노드) 이웃을 취합 → **종목 다각도 집계**(같은 종목의 내러티브별 다른 파급) → **종목별 재분석+콜**(연쇄 sonnet ×M — 펀더[캐시 업사이드]·심리[언급 모멘텀]·기술[RS·52주] 종합 → 상승여력·불안신호 → **레이팅** `_rating`: 상승여력 ≥50 Strong Buy·≥15 Buy·이하 Hold·불안신호 Sell) → **Top-down 설득형 리포트**(opus: 투자 포인트→산업 내러티브→Numbers→종목[레이팅]→리스크, 2000~2600자, 매수 일변도 금지). **② 보강**: 파급 없는 구성 내러티브는 상한 `AUGMENT_CAP`(2)만큼 scenario 즉석 생성(best-effort). `reports` 캐시(members_hash 멱등, stocks_json에 레이팅). 없는 사실 창작 금지(mega 규율) |
...
| `GET /api/spine/report?topic=` · `POST /api/spine/report/compute?topic=&refresh=` | **통합 리포트**(integrated-report) 조회(저장분, LLM 0) / 생성(연쇄 LLM: 공유 이웃 취합→종목 다각도 재분석 sonnet×M→Top-down opus, 구성원 해시 동일하면 저장분·refresh만 재생성). 내러티브 페이지 '통합 리포트' 섹션에 노출 |
