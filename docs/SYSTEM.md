...
| `GET /api/spine/stock/{code}/lens` · POST `/lens/compute?type=&refresh=` | **투자 렌즈(D-090)** — 캐시 판독 번들(value/trend)+**4상한(quadrant)**+stale(LLM 0) / 원칙·재료 변경 시만 생성(sonnet, `type=value\|trend`, 멱등). 재료·원칙 없는 렌즈는 응답에서 생략. 가치·추세 KR |
