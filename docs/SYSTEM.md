...
| `questions` | **핵심질문 트래커(D-067·D-068, docs/specs/question-proxy.md)** — `decompose_question`(질문→서브질문·프록시 분해, sonnet)·`rollup`(프록시 관측을 pace layer 2층으로 **결정적** 롤업 — 선행 lead[sentiment·stance]/확정 confirm[numeric]/divergence, 판정 변할 때만 haiku 한 줄 `verdict_summary`)·`get_tree`·`list_questions`. 프록시의 `yes_direction`으로 관측 방향→'예/아니오' 극성 매핑. numeric 프록시는 분해 시 `transcript.extract_proxies` 편승 추출(event-driven, 고정 폴러 없음). 생성자 ②(사용자 주입) Phase 1 본진, ①(내러티브 자동도출)·sentiment/stance·observations 투영은 Phase 2 |
...
| `GET·POST·DELETE /api/spine/questions` (+`/{id}`·`/{id}/rollup`) | **핵심질문 트래커(D-067·D-068)** — POST(질문 주입→자동분해→numeric 프록시 추출→트리, ~수 분) · GET(미결 질문 목록, LLM 0) · `/{id}`(트리: 서브질문→프록시→관측 시계열+2층 판정, LLM 0) · POST `/{id}/rollup`(판정 재롤업, 결정적·판정 변화 시만 haiku) · DELETE(dismiss) |
