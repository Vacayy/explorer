...
| `notify` / `bot` | **텔레그램 아침 브리핑 + 양방향 봇(D-107, docs/specs/telegram-briefing.md)** — `notify.compose_briefing()`이 본문+inline keyboard를 조립(LLM 0: 홈 3줄·미국장 캐시·어제의 주제[최다3+급상승2, THEME_STOPWORDS 제외 필수]·팔로우 유튜브 3일), `push_briefing`이 발송+`job_runs` 기록, `ensure_briefing_sent`이 체인 편승 캐치업(D-106). `bot`은 FastAPI startup 데몬 스레드로 long-polling — 종목명/RAG 질문/`/briefing` 외에 **브리핑 버튼 콜백**(`n:{entity_id}`→`compute_narrative`, `y:{doc_id}`→유튜브 lazy digest)을 즉시 ack+선응답 후 데몬 스레드에서 실행(`_inflight` 중복 기동 잠금, 4,000자 초과는 문단 분할). **제약**: 발송은 launchd라 서버 무관이나 콜백은 서버가 떠 있을 때만 — 꺼져 있으면 getUpdates 백로그(24h)에 남아 켜질 때 처리(유실 없음, 즉시성 없음) |
