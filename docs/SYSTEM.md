...
| `report` | **통합 리포트**(integrated-report) — 앵커 주제 + `related_narratives`(공유 인과 노드) 이웃을 애널리스트 참고자료로 취합 → **종목 다각도 집계**(같은 종목의 내러티브별 다른 파급을 캐시된 scenario에서 모음) → **종목별 재분석**(연쇄 sonnet ×M, 다각도 관통 투자 포인트) → **Top-down 리포트 종합**(opus: 산업 분석→기업 분석→투자 포인트·전략, 범위+조건부·무효화 조건·추세추종 타이밍 분리). `reports` 캐시(members_hash 멱등). LLM 없는 취합/집계 + 연쇄 LLM M+1콜. 없는 사실 창작 금지(mega 규율) |
