...
| `transcript_follow` / `transcripts` / `proxy_registry` / `proxy_observations` | transcript(D-061) | 미국 기업 컨콜 팔로우(21종 시드)+얇은 인덱스(raw_doc_id FK·**digest**=LLM 핵심 정리) / 프록시 레지스트리(사람 세팅)·관측치(컨콜에서 haiku 추출한 값·방향 시계열). 전문은 raw_documents(source_type='transcript'), 인과·임베딩은 기존 파이프라인 경유 |
...
| `GET /api/spine/report?topic=`(최신) · `/history?topic=`(버전 목록) · `/version?id=`(과거 버전) · `POST /report/compute?topic=&refresh=` | **통합 리포트**(integrated-report) 조회/히스토리(append-only, D-047) / 생성(연쇄 LLM: 공유 이웃 취합→종목 다각도 재분석 sonnet×M→Top-down opus). 내러티브 인라인 + 리포트 탭(ReportView)에 노출, 과거 버전 열람·Top-pick 강조 |
