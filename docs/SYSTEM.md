...
| `GET /api/spine/us/{ticker}` | **미국 종목 도시에 헤더(D-091)** — yfinance 시세·밸류(us_data.get_fundamentals) + resolve된 entity_id + 최근 컨콜 메타. LLM 0(캐시). 렌즈는 `/lens?market=us`, 컨콜·내러티브·피드는 각 전용 엔드포인트 재사용 |
...
미국(/us/:ticker)   **미국 종목 경량 도시에(D-091)** — 헤더(yfinance 시세·밸류) + 투자 렌즈(가치/추세, market='us') + 컨콜·언급 링크. 컨콜 팔로우 상세 '도시에 →'로 진입. `/analyze`(DART 한국 전용)와 분리
