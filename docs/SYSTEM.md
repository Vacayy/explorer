...
| `GET·POST·DELETE /api/spine/questions` (+`/propose`·`/{id}`·`/{id}/rollup`·`/{id}/approve`) | **핵심질문 트래커(D-067·D-068·D-069)** — POST(질문 주입→자동분해→numeric+sentiment 추출→트리, ~수 분) · GET(미결 목록, `narrative_id`·`status` 필터, LLM 0 — 내러티브 미러링·제안 큐용) · `/{id}`(트리: 서브질문→프록시→관측+2층 판정, LLM 0) · POST `/propose`(지배 내러티브서 질문 후보 도출, 생성자 ①) · POST `/{id}/approve`(제안 승인→분해·추적) · POST `/{id}/rollup`(재롤업) · DELETE(dismiss) |
