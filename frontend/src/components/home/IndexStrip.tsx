// Home 최상단 주요 지수 스트립 — 미국·한국·홍콩·일본 종가·전일대비·1Y 추이 (D-110)
...
export function IndexStrip() {
  ...
  if (isLoading) {
    return (
      <Card>
        <CardContent className="grid grid-cols-2 gap-4 py-3 sm:grid-cols-4 lg:grid-cols-7">
    ...
    <Card>
      <CardContent className="space-y-2 py-3">
          ...
          {items.map((it) => (
            <div key={it.key} className="min-w-0">
              <div className="flex items-baseline gap-1">
                ...
                <span className="shrink-0 text-[10px] text-muted-foreground">{it.region}</span>
              </div>
              {it.price === null ? (
                ...
                <>
                  <div className="mt-0.5 flex items-baseline gap-1.5">
                    <span className="text-sm font-semibold tabular-nums">{formatNumber(it.price)}</span>
                    <span
                  ...
                  <div className="mt-0.5 space-y-px text-[10px] leading-tight text-muted-foreground">
                    <div className="tabular-nums">
                      {it.hours_kst}
                      {it.is_open && <span className="ml-1 text-[var(--color-chart-profit)]">장중</span>}
                    </div>
                    <div
        ...
        <div className="flex flex-wrap items-center gap-x-2 text-[10px] text-muted-foreground">
          ...
          <span>· {data?.any_open ? "개장 중 10분 주기 갱신" : "전 시장 휴장 (30분 주기 확인)"}</span>
          ...
          {data?.degraded && data.degraded.length > 0 && (
            <span className="flex items-center gap-1 text-hypothesis">
              <AlertTriangle className="h-3 w-3" /> 수집 실패: {data.degraded.join(", ")}
            </span>
          )}
        </div>
      </CardContent>
